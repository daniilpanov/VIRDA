...
class TestStage2Config:
    def test_defaults(self) -> None:
        config = Stage2Config()
        assert config.neighborhood_radius_mm == 10.0
        assert config.k_neighbors is None
        assert config.use_weighted_pca is False
        assert config.pca_sigma_mm == 5.0
    ...
    def test_accepts_none_k(self) -> None:
        assert Stage2Config(k_neighbors=None).k_neighbors is None

...
class TestAutoFiducialsDetector:
    def test_detector_returns_auto_detected_store(self) -> None:
        mesh = _head_mesh()

        result = AutoFiducialsDetector().run(build_context(mesh=mesh))

        assert isinstance(result, AutoDetectedFiducials)

...
class TestStage3Exporter:
    def test_exports_electrodes_json(self, tmp_path) -> None:
        ...
        data = json.loads((tmp_path / "stage3" / "electrodes.json").read_text())
        assert len(data) == 2
    ...
    def test_exports_csv(self, tmp_path) -> None:
        ...
        with (tmp_path / "stage3" / "electrode_coords.csv").open() as f:
            rows = list(csv.DictReader(f))
        assert rows[0]["electrode_id"] == "E0"
        assert result.items[0].scalp_coords is not None
        assert float(rows[0]["x"]) == pytest.approx(result.items[0].scalp_coords[0])
        assert float(rows[0]["y"]) == pytest.approx(result.items[0].scalp_coords[1])
        assert float(rows[0]["z"]) == pytest.approx(result.items[0].scalp_coords[2])
